...
def check_events(date: str) -> str:
    ...
    if count > 5:
        lines.append(f"  ...and {count - 5} more event{'s' if count - 5 > 1 else ''}.")
    return "\n".join(lines)
...
def delete_event(event_id: str) -> str:
    ...
    try:
        ...
        return "Could not delete the calendar event right now."
...
def update_event(event_id: str, title: str, start_datetime: str, end_datetime: str) -> str:
    ...
    if not service:
        return "Google Calendar is not configured."
    try:
        start_time = dt.fromisoformat(start_datetime)
        end_time = dt.fromisoformat(end_datetime)
    except ValueError:
        return "Invalid datetime format. Use YYYY-MM-DDTHH:MM:SS."
    ...
    try:
        ...
        return "Could not update the calendar event right now."
...
def get_month_events(year: int, month: int) -> list:
    ...
    start_date = dt(year, month, 1, tzinfo=IST)
    if month == 12:
        end_date = dt(year + 1, 1, 1, tzinfo=IST)
    else:
        end_date = dt(year, month + 1, 1, tzinfo=IST)
